...
Patch01: xfs-inode-log-reservations-still-too-small.patch
...
pushd linux-%{version}-%{release}.%{_target_cpu} > /dev/null
ApplyOptionalPatch xfs-inode-log-reservations-still-too-small.patch
popd > /dev/null
...
dracut -f $RPM_BUILD_ROOT/boot/initrd-%{version}-%{release}.%{_target_cpu}.img %{version}-%{release}.%{_target_cpu}
%{__mv} initrd-%{version}-%{release}.%{_target_cpu}.img $RPM_BUILD_ROOT/boot/initrd-%{version}-%{release}.%{_target_cpu}.img
%else
